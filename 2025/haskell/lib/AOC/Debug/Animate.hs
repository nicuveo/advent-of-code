...
data AnimParams a m = AP
  { renderFun :: RenderFun a
...
data Action = Pause | Faster | Slower | ResetSpeed | Restart | PrevFrame | NextFrame | Help | Quit deriving
    ( Show
    )
...
renderFrame = do
  ...
  liftIO $ do
    clear
    let pl :: String = if paused then " (paused)" else ""
    printf "Interval: %dms%s                 \nFrame: #%d        \n%s" delay pl n a
    forM_ logs $ \l -> clearLine >> putStrLn l
    clearLine

processActions :: MonadIO m => MonadAnim a m ()
processActions = liftIO peekActions >>= traverse_ \case
  Pause -> do
    modify (\s -> s { isPaused = not $ isPaused s })
    renderFrame
  Faster -> do
    modify (\s -> s { currentDelay = max   20 $ round $ fromIntegral (currentDelay s) * (0.80 :: Double) })
    whenM (gets isPaused) renderFrame
  Slower -> do
    modify (\s -> s { currentDelay = min 2000 $ round $ fromIntegral (currentDelay s) * (1.25 :: Double) })
    whenM (gets isPaused) renderFrame
  ResetSpeed -> do
    value <- asks origDelay
    modify $ \s -> s { currentDelay = value }
    whenM (gets isPaused) renderFrame
  Restart    -> whenM (gets isPaused) $ restart       >> renderFrame
  PrevFrame  -> whenM (gets isPaused) $ stepBackwards >> renderFrame
  NextFrame  -> whenM (gets isPaused) $ stepForwards  >> renderFrame
  Help       -> printHelp >> renderFrame
  Quit       -> modify $ \s -> s { canContinue = False }
